...
export function SupplierForm() {
  ...
  return (
    <form action={formAction} className="bg-white border border-slate-200 rounded-lg p-5 space-y-4">
      <h2 className="text-sm font-semibold text-slate-900">New Supplier</h2>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Field label="Supplier Code" name="supplierCode" />
        <Field label="Type" name="type" placeholder="airline, hotel, visa_agent…" />
        <Field label="Phone" name="phone" />
        <Field label="Email" name="email" type="email" error={state.fieldErrors?.email} />
        <Field label="Opening Balance" name="openingBalance" type="number" step="0.01" defaultValue={0} />
      </div>
